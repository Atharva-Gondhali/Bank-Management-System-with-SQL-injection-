...
public class Manager {
	...
	int block_account(int acc_num, String aadhar)
	{
		...
		if ( aadhar.equals(db.get_aadhar(temp_client)) == false)
		{
			return -1; 							//-1 means that the aadhar and account number are of different accounts
		}
	...
	int unblock_account(int acc_num, String aadhar)
	{
		DB_Handler db = new DB_Handler();
		int temp_client = db.get_client_id(acc_num);
		int temp_acc_type = db.get_account_status(acc_num);
	...
	int block_card(int acc_num, String aadhar, String card_no)
	{
		DB_Handler db = new DB_Handler();
		
		int temp_client_id = db.get_client_id(acc_num);
		String temp_cnic = db.get_aadhar(temp_client_id);
		int temp_card_no = db.get_card_num(acc_num);
		
		if (temp_cnic.compareTo(aadhar) != 0)
		{
	...
	int unblock_card(int acc_num, String aadhar, String card_no)
	{
		DB_Handler db = new DB_Handler();
	...
	int close_account(String account_num, String aadhar)
	{
		DB_Handler db = new DB_Handler();	
		int temp_client_id = db.get_client_id(Integer.parseInt(account_num));
		
		if ( aadhar.equals(db.get_aadhar(temp_client_id)) )
		{
	...
	public String getAccNum(String Aadhar) {
		DB_Handler db = new DB_Handler();	
		String acc_num = "";
		acc_num = db.getAccNum(Aadhar);
		return acc_num;
	}
