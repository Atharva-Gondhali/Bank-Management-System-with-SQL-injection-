...
public class DB_Handler {
	...
	private String password = "atharva123";
	...
	public String get_aadhar(int client_id)
	{
		try
		{
	...
	// Check if any account with provided account number and Aadhar is present or not and returning the account if found
	Bank_Account searchAccount1( String accountNum, String Aadhar ) {
		Bank_Account account = new Bank_Account();
		try {	
			// finding the account in database
			String aQuery = "Select * From bank_schema.bank_account Where acc_num = "+accountNum+" and "
					+ " client_id = (select client_id from bank_schema.client where Aadhar=\""+Aadhar+"\")";
			System.out.println(aQuery);
			Statement uaSt = conn.createStatement();
	...
	public String getAccNum(String Aadhar) {
		String b = "";
		try {	
			String aQuery = "Select * From bank_schema.bank_account Where client_id = (select client_id from bank_schema.client where Aadhar=\""+Aadhar+"\")";
			System.out.println(aQuery);
			Statement uaSt = conn.createStatement();
			ResultSet uaRs = uaSt.executeQuery(aQuery);
			if( uaRs.next() ) {
